...
FROM campaign
WHERE outcome = 'live' --will test for active status against current date. (No records are returned due to the data)
...
SELECT c.contact_id, c.first_name, c.last_name, c.email, cp.cf_id, cp.company_name, cp.launch_date, cp.end_date, cp.goal-cp.pledged as "remaining_goal_amount"
...
SELECT b.first_name, b.last_name, b.email, c.cf_id, c.company_name, c.launch_date, c.end_date, c.goal - c.pledged as "left_of_goal"
